...
(function (window) {
  ...
  window.appDOM = function () {
    ...
    const utils = {
      ...
      extractColors: function () {
        ...
        const shades = Array.from({
          length: 17
        }, (_, i) => (i + 1) * 50).filter(shade => shade <= 900);
        const bootstrapVars = [{
          name: "bodyBg",
          style: "background-color: var(--bs-body-bg)"
        }, {
          name: "bodyBgRgb",
          style: "background-color: rgb(var(--bs-body-bg-rgb))"
        }, {
          name: "bodyColor",
          style: "color: var(--bs-body-color)"
        }, {
          name: "bodyColorRgb",
          style: "color: rgb(var(--bs-body-color-rgb))"
        }];
    ...
    plugin.setThemeStyle = function (themeUrl) {
      ...
      let styleElement = document.getElementById(config.theme.styleId);
      if (styleElement) {
        logger.log(`Updating existing theme link: ${sanitizedUrl}`);
        styleElement.href = sanitizedUrl;
      } else {
        logger.log(`Creating new theme link: ${sanitizedUrl}`);
        styleElement = document.createElement('link');
        styleElement.id = config.theme.styleId;
        styleElement.rel = 'stylesheet';
        styleElement.media = 'screen';
        styleElement.href = sanitizedUrl;

        // Append as the last link element
    ...
    const handlers = {
      ...
      themeStyle: function (element) {
        ...
        if (themeUrl === '') {
          ...
          // Set the theme style (THEME_CSS_BASE from Laravel = sa5/css/; else CI4-style admin/css/)
          let finalUrl = themeUrl;
          if (!finalUrl.endsWith('.css')) {
            var base = (typeof THEME_CSS_BASE !== 'undefined' && THEME_CSS_BASE) ? THEME_CSS_BASE : (BASE_URL + 'admin/css/');
            finalUrl = base + finalUrl + '.css';
          }
